#include <stdio.h>

int main() {
    // Coefficients of the line 5x - y = 100
    int A = 5;
    int B = -1;
    int C = 100;

    // Normal vector
    int normal_vector[2] = {A, B};

    // Direction vector (perpendicular to normal)
    int direction_vector[2] = {B, -A};  // Swapping and negating A

    // Print the vectors
    printf("Normal: [%d, %d]\n", normal_vector[0], normal_vector[1]);
    printf("Direction: [%d, %d]\n", direction_vector[0], direction_vector[1]);

    // Print all points on the line in a single line
    printf("Line Points: ");
    for (int x = -30; x <= 30; x++) {
        double y = A * x - C;  // Rearranged line equation to y = 5x - 100
        printf("(%d, %.2f)", x, y);
        if (x < 30) printf(", ");  // Add a comma between points, but not after the last point
    }
    printf("\n");

    return 0;
}

